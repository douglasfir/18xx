    ...
    module G18TS
      class Game < Game::Base
        ...
        ALL_TRAINS = {
          ...
          '2D' => { distance: 2, price: 600, multiplier: 2},
          '7' => { distance: 7, price: 720 },
          'P3' => { distance: 3, price: 370 },
          '3+' => {
            distance: [{ 'nodes' => ['city'], 'pay' => 3, 'visit' => 3 },
                       { 'nodes' => ['town'], 'pay' => 99, 'visit' => 99 }],
            price: 410,
          },
          '3D' => { distance: 3, price: 830, multiplier: 2},
          'P4' => { distance: 4, price: 480 },
        }.freeze
        ...
        def game_trains
          ...
          when '1'
            add_train_list(trains, { '2' => 7, '3' => 6, '4' => 4, '5' => 4,'P3' => 1,'3+' => 1,'6' => 5, '2D' => 1,'7' => 10, 'P4' => 2 })
          when '2'
            add_train_list(trains, { '2' => 5, '3' => 5, '4' => 4, '5' => 4, 'P3' => 2,'3+' => 1,'6' => 3, '7' => 10  })
          else # all units
            # extra 5/3T/U3 for minors
            add_train_list(trains, { '2' => 1, '3' => 1, '4' => 2, '5' => 2, 'P3' => 3, '3+' => 2, '6' => 1, '2D' => 1, '7' => 1, '3D' => 1, 'P4' => 2})
            #real one to add back
            #add_train_list(trains, { '2' => 7, '3' => 6, '4' => 5, '5' => 4, 'P3' => 3, '3+' => 3 ,'6' => 3, '2D' => 2, '7' => 10, '3D' => 2})
          end

          add_train_list(trains, { '3T' => 2, 'U3' => 2 }) if @optional_rules.include?(:uP3)
        ...
        def init_optional_rules(optional_rules)
          ...
          raise OptionError, 'Cannot use extra Unit 3 trains without Unit 3' if !@units[3] && optional_rules.include?(:uP3)
        ...
        def setup
          ...
          # random. Throw in PRR pres shares then PRR reg shares when out of companies (not an issue when
          ...
          @players.each_with_index do |player, idx|
            if idx < size
              ...
              prr = corporation_by_id('PRR')
              price = par_prices(prr).first
              @stock_market.set_par(prr, price)
              share = prr.ipo_shares.first
              @share_pool.buy_shares(player, share.to_bundle, exchange: nil, swap: nil, allow_president_change: true)
              after_par(prr)
            else
              prr = corporation_by_id('PRR')
              share = prr.ipo_shares.find { |s| !s.owner&.player? }
              @share_pool.buy_shares(player, share.to_bundle, exchange: nil, swap: nil, allow_president_change: true)
            end
          end
        ...
        def stock_round
          Engine::Round::Stock.new(self, [
            ...
            G18TS::Step::BuySellParSharesCompanies,
          ])
        end

        def operating_round(round_num)
          G18TS::Round::Operating.new(self, [
            Engine::Step::HomeToken,
            G18TS::Step::TrackAndToken,
            G18TS::Step::Route,
            G18TS::Step::Dividend,
            Engine::Step::DiscardTrain,
            G18TS::Step::BuyTrain,
        ...
        def after_buying_train(train, source)
          return unless new_rank_available?(train, source)
          if @depot.upcoming.first.name == '5'
          end
        end

        def new_rank_available?(train, source)
