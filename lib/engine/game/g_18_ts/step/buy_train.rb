      ...
      module Step
        class BuyTrain < Engine::Step::BuyTrain
          ...
          def buyable_trains(entity)
            ...
            if depot_trains.any? { |t| t.name == '5' }            
              depot_trains += @depot.upcoming.select { |t| t.name == 'P3' || t.name == '3+' }
            end                      
            if depot_trains.any? { |t| t.name == '6' }
              old_size = depot_trains.size           
              depot_trains += @depot.upcoming.select { |t| t.name == 'P3' || t.name == '3+' }
              if depot_trains.size > old_size              
                depot_trains.reject! { |t| t.name == '6' }
              else               
                depot_trains += @depot.upcoming.select { |t| t.name == '2D' }
              end
            end          
            if depot_trains.any? { |t| t.name == '7' }
              old_size = depot_trains.size            
              depot_trains += @depot.upcoming.select { |t| t.name == '2D' }
              if depot_trains.size > old_size               
                depot_trains.reject! { |t| t.name == '7' }
              else              
                depot_trains += @depot.upcoming.select { |t| t.name == '3D' }
              end
            end                    
            depot_trains = depot_trains.reject { |t| entity.cash < t.price }                    
